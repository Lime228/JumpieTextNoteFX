...
public class StyledDocument implements Serializable {
    ...
    public void optimizeStyles() {
        ...
        for (int i = 1; i < styles.size(); i++) {
            ...
            if (current.getEnd() + 1 >= next.getStart() && current.matches(next)) {
                current = new TextStyle(Math.min(current.getStart(), next.getStart()), Math.max(current.getEnd(), next.getEnd()), current.getFontFamily(), current.getFontSize(), current.isBold(), current.isItalic(), current.isUnderline(), current.isStrikethrough());
            } else {
